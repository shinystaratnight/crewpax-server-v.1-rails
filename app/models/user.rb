class User < ActiveRecord::Base
  ...
  validates :phone, format:{with:/\d{10}/, message:" must be a valid phone number"}, 
  length: {maximum: 10}, if: "phone.present?"
  validates :email,length: {maximum: 64}, format:{with: /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\Z/, message:" must be a valid email address"},
  if: "email.present?"
